using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Data.Entity.Infrastructure;
using System.Linq;
using System.Net;
using System.Net.Http;
using System.Text;
using System.Threading.Tasks;
using System.Web.Http;
using System.Web.Http.Description;
using FEL_API.Models;
using FELFactura;
using Modelos;
using Newtonsoft.Json;

namespace FEL_API.Controllers
{
    public class DatosFoxesController : ApiController
    {
        XMLFactura xml = new XMLFactura();
        private FEL_APIContext db = new FEL_APIContext();

        // GET: api/DatosFoxes
        public IQueryable<DatosFox> GetDatosFoxes()
        {
            return db.DatosFoxes;
        }

        // GET: api/DatosFoxes/5
        [ResponseType(typeof(DatosFox))]
        public IHttpActionResult GetDatosFox(int id)
        {
            DatosFox datosFox = db.DatosFoxes.Find(id);
            if (datosFox == null)
            {
                return NotFound();
            }

            return Ok(datosFox);
        }

        // PUT: api/DatosFoxes/5
        [ResponseType(typeof(void))]
        public IHttpActionResult PutDatosFox(int id, DatosFox datosFox)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }

            if (id != datosFox.id)
            {
                return BadRequest();
            }

            db.Entry(datosFox).State = EntityState.Modified;

            try
            {
                db.SaveChanges();
            }
            catch (DbUpdateConcurrencyException)
            {
                if (!DatosFoxExists(id))
                {
                    return NotFound();
                }
                else
                {
                    throw;
                }
            }

            return StatusCode(HttpStatusCode.NoContent);
        }

        // POST: api/DatosFoxes
        [ResponseType(typeof(DatosFox))]
        public async Task<IHttpActionResult> PostDatosFoxAsync(DatosFox datosFox)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }

           String xmlDoc = xml.getXML(datosFox.xml_encabezado, datosFox.xml_detalle, datosFox.path, datosFox.num_fac);

            String res = await hacerAsync("");
            return Ok(xmlDoc);
        }

        // DELETE: api/DatosFoxes/5
        [ResponseType(typeof(DatosFox))]
        public IHttpActionResult DeleteDatosFox(int id)
        {
            DatosFox datosFox = db.DatosFoxes.Find(id);
            if (datosFox == null)
            {
                return NotFound();
            }

            db.DatosFoxes.Remove(datosFox);
            db.SaveChanges();

            return Ok(datosFox);
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }

        private bool DatosFoxExists(int id)
        {
            return db.DatosFoxes.Count(e => e.id == id) > 0;
        }
        private static async Task<string> hacerAsync(String xml)
        {
            var tags = "";
            using (var client = new HttpClient())
            {
                //  XmlDocument xmltest = new XmlDocument();
                //xmltest.LoadXml(xml);

                //Pasar el xml a Base64
                var arrayDeBytes = xml.ToString();
                var encoding = new UnicodeEncoding();
                String s = Convert.ToBase64String(encoding.GetBytes(xml), Base64FormattingOptions.InsertLineBreaks);
                Solictud solictud = new Solictud();
                solictud.llave = Constants.LLAVE_TOKEN;
                solictud.archivo = "";
                solictud.codigo = "155ssss20";
                solictud.alias = Constants.ALIAS;
                solictud.es_anulacion = "N";

                //pasar a json el objeto
                string json = JsonConvert.SerializeObject(solictud);

                client.BaseAddress = new Uri(Constants.URL_SOLICITUD_FIRMA);
                client.DefaultRequestHeaders.Accept.Clear();
                HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, Constants.METODO_SOLICITUD_FIRMA);
                request.Content = new StringContent(json,
                                                    Encoding.UTF8,
                                                    "application/json");

                // Deserealizar respuesta objeto JSON
                RespuestaSolitud respuestaSolitud = new RespuestaSolitud();
                await client.SendAsync(request)
                .ContinueWith(async responseTask =>
                {
                    respuestaSolitud = JsonConvert.DeserializeObject<RespuestaSolitud>(responseTask.Result.Content.ReadAsStringAsync().Result);
                });

                return respuestaSolitud.ToString();
            }
        }
    }
}